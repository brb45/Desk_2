...
MACHINE_ID = "CUSTOMIZE_QA_BRCM4378"
...
# base_path for all test related files
QA_RELATED_PATH = "C:\\Li\\Auto_QA\\Automation_Related\\BRCM4378\\"
QA_FILE_MULTI_DUT = "IQfact_MultiDUT.ini" # flow/apt configure file
QA_FILE_PATHLOSS_DUT_1 = "path_loss_dut1.csv"
QA_FILE_PATHLOSS_DUT_2 = "path_loss_dut2.csv"
QA_FILE_FLOW = "Murata_flow_limits_DUT1_auto_QA.txt"        # Flow under test
...
Sender = "CUSTOMIZE_QA_BRCM4378@litepoint.com"
...
Recipients = ['jian.sun@litepoint.com']
QA_DUT_IP_ARRAY = ["192.168.100.31"]
QA_DUT_USER_ARRAY = ["root"]
QA_DUT_PASSWORD_ARRAY = ["brcm1234"]
QA_DUT_PATH_ARRAY = ["//root//Desktop//4378_FW//4378_LEMUR_18_10_405_4//"]
...
def do_diff(data_string):
    ...
    for dut_id in [1]:     
        ...
        if os.path.exists(golden_log):
            ...
            if os.path.exists(log_need_compare):
                ...
                winmerge_cmd = 'C:\\WinMerge\\WinMergeU.exe.lnk ' + '\"' + golden_log + '\"' + ' ' + '\"' + log_need_compare + '\"' + ' -minimize -noninteractive -u -or ' + '\"' + tmp_html + '\"'
    ...
    if (os.path.isfile(QA_RELATED_PATH+QA_FILE_FLOW) ):
        ...
        if ( os.path.isfile( data_string+TEST_FLOW_UPDATED)):
            ...
            winmerge_cmd = 'C:\\WinMerge\\WinMergeU.exe.lnk ' + '\"' + (QA_RELATED_PATH+QA_FILE_FLOW) + '\"' + ' ' + '\"' + (data_string+TEST_FLOW_UPDATED) + '\"' + ' -minimize -noninteractive -u -or ' + '\"' + flow_tmp_html + '\"'
...
def send_email(data_string, url_address_array, iqramp_url_array):
    ...
    msg['Subject'] = "=?utf-8?Q?=E2=9C=85=E2=9C=85=E2=9C=85?= Pass Customize : Data Driven QA BRCM4378 1 DUT"
    msg['From'] = Sender
