...
def read_test_data_from_csv():
    test_data = []
    with open('..\\test_data\\test_data_zip_codes.csv', newline='') as csvfile:
...
# @pytest.fixture(name='csv_data_origin')
# def read_test_data_from_csv_1():
#     test_data = []
#     with open('..\\test_data\\test_data_zip_codes.csv', newline='') as csvfile:
#         data = csv.reader(csvfile, delimiter=',')
#         next(data)  # skip header row
...
#     return test_data
#
# @pytest.fixture(name='csv_data', params=read_test_data_from_csv_1())
# def pass_csv_data(request):
#     return request.param
#
#
# def test_using_csv_get_location_data_check_place_name_1(csv_data):
#     (country_code, zip_code, expected_place_name) = csv_data
#     response = requests.get(f"http://api.zippopotam.us/{country_code}/{zip_code}")
#     response_body = response.json()
#     print(country_code, zip_code, expected_place_name)
#     assert response_body["places"][0]["place name"] == expected_place_name